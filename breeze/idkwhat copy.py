...
todays_date_format1 = "29-Dec-2023"
strike = 48300
right_format1 = "CE"  #"PE" for put
right_format2 = "call"
sltp_expiry = "03-Jan-2024"
expiry = "2024-01-03T16:00:00.000Z"
sltp_stock = "CNXBAN"
stock = "CNXBAN"
validity_date = "2023-12-29T16:00:00.000Z"
...
def get_limit_rate(close):
  if(stoploss_margin*close >= 60.05):
     return 0.95*stoploss_margin*close
  elif(stoploss_margin*close >= 30.05):
     return 0.82*stoploss_margin*close
  elif(stoploss_margin*close >= 20.05):
     return 0.8*stoploss_margin*close
  elif(stoploss_margin*close >= 15.05):
     return 0.6*stoploss_margin*close
  elif(stoploss_margin*close >= 10.05):
     return 0.4*stoploss_margin*close
  else:
     return 0
...
def on_ticks(ticks):
    ...
    # limit_prices_thread = None
    # limit_prices_thread = threading.Thread(target=get_limit_rate, args=(curr_price))
    # limit_prices_thread.start()
    time += 1
    print("Current Price :", str(curr_price), "at local time", now.second, "and server time", ticks['ltt'][-7:-5], "and the limit rate is", limit_rate_calculated)

    # If too much delay in live data
    if(int(now.second)!=0 and int(now.second)!=1 and int(ticks['ltt'][-7:-5]) - int(now.second) <= -3):
        stop_threads = True
    ...
    if curr_price > np.ma.average(np.array(history), weights = weights) + c_std*np.std(np.array(history)) and order_qty < max_order_qty:
        # Place buy order
        order_time = datetime.now()
        limit_rate_calculated = get_limit_rate(curr_price) 
        fresh_order = (breeze.place_order(stock_code=str(stock),
                        exchange_code="NFO",
                        product="optionplus",
                        action="buy",
                        order_type="market",    ##
                        stoploss=str(stoploss_margin*curr_price),
