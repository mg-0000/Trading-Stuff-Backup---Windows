...
def get_derivative_lookback_put(current, lookback, looks, index):
    if(len(looks)<lookback):
        looks.append(current)
        return 0
    else:
...
def on_ticks(ticks_call, ticks_put, lookback):
    ...
    derivative_put = get_derivative_lookback_put(other, lookback, puts, index_put)
...
def get_path(stock, expiry, right, strike, date, time_interval):
    return '/home/mgoel/Documents/quant_algos/Data/'+ stock + '_options_' + expiry + '_' + right + str(strike) + '_' + time_interval + str(date) + 'T09:15:00.csv'

def get_data(stock, expiry, right, strike, date, time_interval):
    path = get_path(stock, expiry, right, strike, date, time_interval)
    if(os.path.isfile(path)==False):
        historical_data.get_option_historical_data(start_date = str(date) + "T09:15:00.000Z", end_date = str(date) + "T15:30:00.000Z", expiry = expiry + "T07:00:00.000Z", strike = str(strike), stock_code=stock, time_interval=time_interval, right=right)
    return path
...
def get_price(stock, expiry, right, strike, date, time_interval, lookback):
    ...
    return [[strat1_call.net_pl, strat1_put.net_pl], [strat2_call.net_pl, strat2_put.net_pl], [strat3_call.net_pl, strat3_put.net_pl], [strat4_call.net_pl, strat4_put.net_pl], [strat5_call.net_pl, strat5_put.net_pl],[strat6_call.net_pl, strat6_put.net_pl], [strat7_call.net_pl, strat7_put.net_pl], [strat8_call.net_pl, strat8_put.net_pl], [strat9_call.net_pl, strat9_put.net_pl]]
...
def get_spot_price(stock, expiry, right, date, time_interval, lookback = 10):
    if(date!="2023-08-10"):
        path = '/home/mgoel/Documents/quant_algos/Data/' + stock + '_1day' + str(date) + 'T09:15:00.csv'
        if(os.path.isfile(path)==False):
            historical_data.get_equity_historical_data(start_date = str(date) + "T09:15:00.000Z", end_date = str(date) + "T15:30:00.000Z", stock_code=stock, time_interval="1day")
        df = pd.read_csv(path)
    ...
    return get_price(stock = stock, expiry = expiry, right = right, strike = spot, date = date, time_interval = time_interval, lookback = lookback)

lookback = np.arange(2, 30, 1)
dates = ["2023-08-24", "2023-08-23", "2023-08-22", "2023-08-21", "2023-08-18"]
expiry = ["2023-08-24", "2023-08-24", "2023-08-24", "2023-08-24", "2023-08-24"]
for i in range(len(dates)):
    ...
    for l in range(2,30):
        result = get_spot_price(stock = 'CNXBAN', expiry = expiry[i], right = 'call', date = dates[i], time_interval = '1second', lookback = l)
        result_strat1_call.append(result[0][0])
        result_strat1_put.append(result[0][1])
        result_strat2_call.append(result[1][0])
        result_strat2_put.append(result[1][1])
        result_strat3_call.append(result[2][0])
        result_strat3_put.append(result[2][1])
        result_strat4_call.append(result[3][0])
        result_strat4_put.append(result[3][1])
        result_strat5_call.append(result[4][0])
        result_strat5_put.append(result[4][1])
        result_strat6_call.append(result[5][0])
        result_strat6_put.append(result[5][1])
        result_strat7_call.append(result[6][0])
        result_strat7_put.append(result[6][1])
        result_strat8_call.append(result[7][0])
        result_strat8_put.append(result[7][1])
        result_strat9_call.append(result[8][0])
        result_strat9_put.append(result[8][1])

        result_strat1.append(result[0][0] + result[0][1])
        result_strat2.append(result[1][0] + result[1][1])
        result_strat3.append(result[2][0] + result[2][1])
        result_strat4.append(result[3][0] + result[3][1])
        result_strat5.append(result[4][0] + result[4][1])
        result_strat6.append(result[5][0] + result[5][1])
        result_strat7.append(result[6][0] + result[6][1])
        result_strat8.append(result[7][0] + result[7][1])
        result_strat9.append(result[8][0] + result[8][1])

    plt.figure(1)
    plt.subplot(331)
    plt.plot(lookback, result_strat1_call, label = 'call')
    plt.plot(lookback, result_strat1_put, label = 'put')
    plt.subplot(332)
    plt.plot(lookback, result_strat2_call, label = 'call')
    plt.plot(lookback, result_strat2_put, label = 'put')
    plt.subplot(333)
    plt.plot(lookback, result_strat3_call, label = 'call')
    plt.plot(lookback, result_strat3_put, label = 'put')
    plt.subplot(334)
    plt.plot(lookback, result_strat4_call, label = 'call')
    plt.plot(lookback, result_strat4_put, label = 'put')
    plt.subplot(335)
    plt.plot(lookback, result_strat5_call, label = 'call')
    plt.plot(lookback, result_strat5_put, label = 'put')
    plt.subplot(336)
    plt.plot(lookback, result_strat6_call, label = 'call')
    plt.plot(lookback, result_strat6_put, label = 'put')
    plt.subplot(337)
    plt.plot(lookback, result_strat7_call, label = 'call')
    plt.plot(lookback, result_strat7_put, label = 'put')
    plt.subplot(338)
    plt.plot(lookback, result_strat8_call, label = 'call')
    plt.plot(lookback, result_strat8_put, label = 'put')
    plt.subplot(339)
    plt.plot(lookback, result_strat9_call, label = 'call')
    plt.plot(lookback, result_strat9_put, label = 'put')
    # plt.show()
    plt.savefig('/home/mgoel/Documents/quant_algos/derivative/figures/' + dates[i] + '.png')

    plt.clf()

    plt.figure(2)
    plt.subplot(331)
    plt.plot(lookback, result_strat1, label = 'combined')
    plt.subplot(332)
    plt.plot(lookback, result_strat2, label = 'combined')
    plt.subplot(333)
    plt.plot(lookback, result_strat3, label = 'combined')
    plt.subplot(334)
    plt.plot(lookback, result_strat4, label = 'combined')
    plt.subplot(335)
    plt.plot(lookback, result_strat5, label = 'combined')
    plt.subplot(336)
    plt.plot(lookback, result_strat6, label = 'combined')
    plt.subplot(337)
    plt.plot(lookback, result_strat7, label = 'combined')
    plt.subplot(338)
    plt.plot(lookback, result_strat8, label = 'combined')
    plt.subplot(339)
    plt.plot(lookback, result_strat9, label = 'combined')
    # plt.show()
    plt.savefig('/home/mgoel/Documents/quant_algos/derivative/figures/' + dates[i] + '.png')
